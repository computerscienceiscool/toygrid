...
echo "[INFO] Starting Yjs WebSocket server on port 1234..."
cd ~/yjs-server || { echo "[ERROR] Could not find ~/yjs-server"; exit 1; }
PORT=1234 node server.js > yjs.log 2>&1 &
YJS_PID=$!
...
REACT_APP_YJS_WEBSOCKET_SERVER_URL=ws://localhost:1234 nohup npm start > toygrid.log 2>&1 &
...
xdg-open http://localhost:3000 > /dev/null 2>&1 || echo "[WARN] Could not open browser"
